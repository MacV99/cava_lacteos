...
Port de `whatsapp-cloud/src/errors.js`. Lo consume el modal del panel vía /api/errors.json.
...
ERROR_MAP = {
    ...
    131053: {"titulo": "No se pudo enviar la media", "explica": "WhatsApp rechazó el archivo (formato no soportado, dañado o muy grande). Sin ffmpeg las notas de voz del navegador pueden fallar; fotos/videos/PDF normales sí funcionan.", "accion": "Reintenta. Si persiste solo en audios, mira el detalle \"Meta dice:\" abajo.", "gravedad": "warn"},
...
def explain_error(code) -> dict:
    return ERROR_MAP.get(code, ERROR_FALLBACK)


def full_message(e) -> str:
    """Motivo crudo completo de un error de Meta: mensaje + detalle (title)."""
    title = getattr(e, "title", None)
    msg = getattr(e, "message", None) or str(e)
    return f"{msg} — {title}" if title and title != msg else msg
